...
class LK21(object):
    servers = ["http://173.212.248.78/"]
    sandbox = "allow-scripts allow-same-origin"

    def search(self, query,page=1):
        headers = {
            "authority": "173.212.248.78/",
            "accept": "*/*",
            "accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7",
            "referer": "http://173.212.248.78/",
            ...
            try:
                ...
                for article in articles:
                    ...
                    detailLink = base64.b64encode(
                        json.dumps({"link": link, "provider": "LK21"}).encode()
                    ).decode("utf-8")
                    if "/tv/" in link:
                    ...
                    result.append(
                        {
                            "link": "/api/get?link=" + link + "&provider=LK21",
                            "detail": detailLink,
    ...
    def getMovies(self, url):
        ...
        for link in links:
            ...
            streamLinks.append({"link":_raw,"detail": "/iframe?link=" + _url + "&provider=LK21", "title": link.text})
    ...
    def getSeries(self, url):
        ...
        for link in links:
            epsLinks.append(
                {
                    "link": "/api/get?link=" + link["href"] + "&provider=LK21",
                    "detail": "/detail?detail="
                    + base64.b64encode(
                        json.dumps(
                            {"link": link["href"], "provider": "LK21"}
                        ).encode()
                    ).decode("utf-8"),
                    "title": link.text,
                }
